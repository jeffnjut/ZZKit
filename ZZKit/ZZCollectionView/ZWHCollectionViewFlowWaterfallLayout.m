...
-(void)prepareLayout{
    ...
    for (NSInteger section = 0; section < numberOfSections; section++) {
        ...
        for (NSInteger item = 0; item < itemCountOfSection; item++) {
            ...
            self.sectionColumnHeightArray[section][minIndex] = @(CGRectGetMaxY(attributess.frame) + minimumLineSpacing);
        }
        ...
        if (((NSArray *)self.sectionColumnHeightArray[section]).count > 0) {
            top = [self.sectionColumnHeightArray[section][columnIndex] floatValue] - minimumLineSpacing + sectionInset.bottom;
        } else {
            top = 0;
        }
