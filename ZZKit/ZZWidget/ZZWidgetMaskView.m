...
@implementation ZZWidgetMaskView
...
- (void)showInView:(UIView *)view {
    ...
    self.alpha = 0.f;
    switch (self.maskCoverType) {
        case ZZWidgetMaskViewCoverTypeColored:
            self.bgImg = [self.class imageWithColor:self.coverColor];
            break;
        case ZZWidgetMaskViewCoverTypeBlurred:
    ...
    [UIView animateWithDuration:self.showDuration delay:0 options:UIViewAnimationOptionCurveLinear animations:^{ weakSelf.alpha = 1.0f; } completion:nil];
}
...
- (instancetype)initWithBgColor:(UIColor *)bgColor revealView:(UIView *)revealView revealType:(ZZWidgetMaskViewRevealType)revealType layoutType:(ZZWidgetMaskViewLayoutType)layoutType {
    
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        [self commonSetupWithMaskCoverType:ZZWidgetMaskViewCoverTypeColored revealType:revealType revealView:revealView layoutType:layoutType];
...
- (instancetype)initWithBlurRadius:(CGFloat)blurRadius revealView:(UIView *)revealView revealType:(ZZWidgetMaskViewRevealType)revealType layoutType:(ZZWidgetMaskViewLayoutType)layoutType {
    
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        [self commonSetupWithMaskCoverType:ZZWidgetMaskViewCoverTypeBlurred revealType:revealType revealView:revealView layoutType:layoutType];
...
- (instancetype)initWithRevalView:(UIView *)revalView layoutType:(ZZWidgetMaskViewLayoutType)layoutType {
    ...
    if (self = [super initWithFrame:[[UIScreen mainScreen]bounds]]) {
        [self commonSetupWithMaskCoverType:ZZWidgetMaskViewCoverTypeColored revealType:ZZWidgetMaskViewRevealTypeRect revealView:revalView layoutType:layoutType];
        [self setupAttachUI];
    }
    return self;
}

- (void)commonSetupWithMaskCoverType:(ZZWidgetMaskViewCoverType)maskCoverType revealType:(ZZWidgetMaskViewRevealType)revealType revealView:(UIView *)revealView layoutType:(ZZWidgetMaskViewLayoutType)layoutType {
    ...
    self.maskCoverType = maskCoverType;
...
- (void)layoutSubviews {
    ...
    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"ZZWidgetMaskView" ofType:@"bundle"];
    
    //others
    switch (self.layoutType) {
        case ZZWidgetMaskViewLayoutTypeUP: {
        ...
        case ZZWidgetMaskViewLayoutTypeLeftUP: {
        ...
        case ZZWidgetMaskViewLayoutTypeLeft: {
        ...
        case ZZWidgetMaskViewLayoutTypeLeftDown: {
        ...
        case ZZWidgetMaskViewLayoutTypeDown: {
        ...
        case ZZWidgetMaskViewLayoutTypeRightDown: {
        ...
        case ZZWidgetMaskViewLayoutTypeRight: {
        ...
        case ZZWidgetMaskViewLayoutTypeRightUP: {
...
- (void)neverBtnClicked:(UIButton *)btn {
    ...
    if (self.delegate != nil ) {
        @try {
            [self.delegate maskViewDidClickedNeverBtn:self];
...
- (void)dismissBtnClicked:(UIButton *)btn {
    ...
    if (self.delegate != nil ) {
        @try {
            [self.delegate maskViewDidClickedDismissBtn:self];
        }@catch(NSException *ex) {}
    }
}
...
@interface ZZWidgetMaskQueue () <ZZWidgetMaskViewDelegate>
...
@implementation ZZWidgetMaskQueue
...
- (void)addPromptMaskView:(ZZWidgetMaskView *)aMask {
    ...
    NSAssert(aMask, @"inset nil object to coverQueue");
    NSAssert([aMask isKindOfClass:[ZZWidgetMaskView class]], @"inset no-promptCoverView to coverQueue");
...
- (void)_showMaskAtIndex:(NSUInteger)index {
    
    ZZWidgetMaskView *mask = _masks[index];
    if (index == 0) {
        mask.dismissDuration = 0;
        mask.dismissBtnTitle = @"下一步";
    } else if (index == (_masks.count - 1)) {
        mask.showDuration = 0;
        mask.dismissBtnTitle = @"完成";
    } else {
        mask.showDuration = 0;
        mask.dismissDuration = 0;
        mask.dismissBtnTitle = @"下一步";
    }
    [mask showInView:self.foundationView];
    _showingMask = mask;
}

#pragma mark - ZZWidgetMaskViewDelegate

- (void)maskViewDidClickedDismissBtn:(ZZWidgetMaskView *)maskView {
    
    NSUInteger index = [_masks indexOfObject:maskView];
    _showingMask = nil;
    if (index == _masks.count - 1) {
        [self dismissAllMasks];
        if (self.delegate != nil) {
            @try {
                [self.delegate maskViewQueueDidDismissAllMasks:self];
            }@catch(NSException *ex) {}
        }
        return;
    }
    if (self.delegate != nil) {
        @try {
            [self.delegate maskViewQueue:self didDismissMaskAtIndex:index];
        }@catch(NSException *ex) {}
    }
    NSUInteger nextIndex = index + 1;
    [self _showMaskAtIndex:nextIndex];
}
...
- (void)maskViewDidClickedNeverBtn:(ZZWidgetMaskView *)maskView {
    ...
    if (self.delegate != nil ) {
        @try {
            [self.delegate maskViewQueue:self didClickedNeverBtnInMaskView:maskView];
        }@catch(NSException *ex) {}
    }
}
