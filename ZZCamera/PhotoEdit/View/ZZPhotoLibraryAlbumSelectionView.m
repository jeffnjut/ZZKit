//
//  ZZPhotoLibraryAlbumSelectionView.m
//  ZZCamera
//
...
#import "ZZPhotoLibraryAlbumCell.h"
#import "ZZDevice.h"
#import "ZZMacro.h"

@interface ZZPhotoLibraryAlbumSelectionView()
...
@implementation ZZPhotoLibraryAlbumSelectionView
...
+ (ZZPhotoLibraryAlbumSelectionView *)create:(CGPoint)point maxColumn:(NSUInteger)maxColumn photoAssetCollections:(NSArray<PHAssetCollection *> *)photoAssetCollections selectedPhotoAssetCollection:(PHAssetCollection *)selectedPhotoAssetCollection assetCollectionChangedBlock:(void(^)(PHAssetCollection *currentCollection))block {
    ...
    ZZPhotoLibraryAlbumSelectionView *view = [[ZZPhotoLibraryAlbumSelectionView alloc] initWithFrame:CGRectMake(point.x, point.y, UIScreen.mainScreen.bounds.size.width, h)];
    ...
    if (_tableView == nil) {
        ...
        _tableView = [ZZTableView zz_quickAdd:ZZTableViewCellEditingStyleNone backgroundColor:[UIColor whiteColor] onView:self frame:CGRectZero constraintBlock:^(UIView * _Nonnull superView, MASConstraintMaker * _Nonnull make) {
            ...
            if (action == ZZTableViewCellActionTapped) {
                if ([cellData isKindOfClass:[ZZPhotoLibraryAlbumCellDataSource class]]) {
                    ZZPhotoLibraryAlbumCellDataSource *ds = cellData;
                    for (ZZPhotoLibraryAlbumCellDataSource *data in weakSelf.tableView.zzDataSource) {
                        data.isSelected = NO;
                        if ([data isEqual:ds]) {
                            data.isSelected = YES;
...
- (void)_render {
    ...
    for (PHAssetCollection *collection in self.photoAssetCollections) {
        ZZPhotoLibraryAlbumCellDataSource *ds = [[ZZPhotoLibraryAlbumCellDataSource alloc] init];
