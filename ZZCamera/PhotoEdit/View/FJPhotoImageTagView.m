...
#define FJPhotoImageTagViewHeight (24.0)
#define FJPhotoImageTagViewWidthExceptText (68.0)
...
@property (nonatomic, weak) IBOutlet FJPhotoImageTagPointView *tagPointLeftView;
@property (nonatomic, weak) IBOutlet FJPhotoImageTagPointView *tagPointRightView;
...
+ (FJPhotoImageTagView *)create:(CGPoint)point containerSize:(CGSize)containerSize model:(FJImageTagModel *)model scale:(CGFloat)scale fixEdge:(BOOL)fixEdge autoChangeDirection:(BOOL)autoChangeDirection tapBlock:(void(^)(__weak FJPhotoImageTagView *photoImageTagView))tapBlock movingBlock:(void(^)(UIGestureRecognizerState state, CGPoint point, FJPhotoImageTagView *imageTagView))movingBlock {
    ...
    FJPhotoImageTagView *view = ZZ_LOAD_NIB(@"FJPhotoImageTagView");
    view.model = model;
    ...
    if (model.v.intValue == 0) {
        ...
        if (model.direction == 0) {
            ...
            point.x -= FJPhotoImageTagViewWidthExceptText + w - 4.0;
            point.y -= 12.0;
        }
    }
