...
@interface FJPhotoScrollView : UIScrollView

@end

@implementation FJPhotoScrollView
...
// ScrollView
@property (nonatomic, strong) FJPhotoScrollView *scrollView;
// Cropper View
...
- (void)dealloc {
    NSLog(@"Dealloc - FJPhotoDraftHistoryViewController");
}
...
- (UIImageView *)deletionImageView {
    ...
    if (_deletionImageView == nil) {
        ...
        _deletionImageView.image = @"FJPhotoEditViewController.ic_tag_garbage".zz_image;
...
- (void)_buildUI {
    ...
    // ScrollView
    if (_scrollView == nil) {
        _scrollView = [[FJPhotoScrollView alloc] initWithFrame:CGRectMake(0, 0, ZZDevice.zz_screenWidth, ZZDevice.zz_screenHeight - ZZ_DEVICE_NAVIGATION_TOP_HEIGHT - _toolbar.bounds.size.height)];
...
- (void)_touch:(nonnull UIImageView *)imageView point:(CGPoint)point {
    ...
    // 限制一张图片上的Tag个数
    if (self.limitedTagCnt > 0) {
        int tagsCnt = 0;
        for (FJPhotoImageTagView *tagView in imageView.subviews) {
            if ([tagView isKindOfClass:[FJPhotoImageTagView class]]) {
                tagsCnt++;
            }
        }
        if (tagsCnt >= self.limitedTagCnt) {
            [self.view zz_toast:[NSString stringWithFormat:@"每张图片最多可以添加 %lu 个标签", (unsigned long)self.limitedTagCnt] toastType:ZZToastTypeWarning];
            return;
        }
    }
    ...
    if ([imageView isKindOfClass:[UIImageView class]]) {
        if ([self.userTagController isKindOfClass:[FJPhotoUserTagBaseViewController class]]) {
            ((FJPhotoUserTagBaseViewController *)self.userTagController).point = point;
...
- (void)_addImageTagOnImageView:(UIImageView *)imageView tag:(FJImageTagModel *)tag point:(CGPoint)point {
    ...
    FJPhotoImageTagView *tagView = [FJPhotoImageTagView create:point containerSize:imageView.bounds.size model:tag fixEdge:YES autoChangeDirection:YES tapBlock:^(__weak FJPhotoImageTagView * photoImageTagView) {
