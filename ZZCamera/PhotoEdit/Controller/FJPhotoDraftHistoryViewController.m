...
- (FJPhotoDraftBottomView *)bottomView {
    ...
    if (_bottomView == nil) {
        _bottomView = ZZ_LOAD_NIB(@"FJPhotoDraftBottomView");
        ...
        _bottomView.deleteBlock = ^{
            for (FJPhotoDraftCellDataSource *ds in weakSelf.tableView.zzDataSource) {
                if (ds.selected == YES) {
...
- (void)_leftButtonAction {
    ...
    if (self.leftButton.tag == 0) {
        ...
        // 点击取消
        for (FJPhotoDraftCellDataSource *ds in self.tableView.zzDataSource) {
            ds.editable = NO;
        }
