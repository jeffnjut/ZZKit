...
@interface ZZPhotoLibraryAlbumCell()
...
@implementation ZZPhotoLibraryAlbumCell
...
- (void)setZzData:(__kindof ZZTableViewCellDataSource *)zzData {
    ...
    ZZPhotoLibraryAlbumCellDataSource *ds = zzData;
    // 获取当前相册 所有照片对象
    ...
    if (@available(iOS 13.0, *)) {
        [firstAsset zz_imageAsyncTargetSize:CGSizeMake(150.0, 150.0) fast:NO iCloud:YES progress:nil result:^(UIImage *image) {
            [weakSelf.albumImage setImage:image];
        }];
    }else {
        [firstAsset zz_imageAsyncTargetSize:CGSizeMake(150.0, 150.0) fast:YES iCloud:YES progress:nil result:^(UIImage *image) {
            [weakSelf.albumImage setImage:image];
        }];
    }
...
@implementation ZZPhotoLibraryAlbumCellDataSource
