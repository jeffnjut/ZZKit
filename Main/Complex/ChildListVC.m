...
- (void)viewDidLoad {
    ...
    self.tableView = [ZZTableView zz_quickAdd:ZZTableViewCellEditingStyleNone
        ...
        if ([weakSelf.subScrollDelegate respondsToSelector:@selector(subScrollViewDidScroll:)]) {
            [weakSelf.subScrollDelegate subScrollViewDidScroll:tableView];
        }
    }];
    if (self.tableName.length > 0) {
        self.tableView.name = self.tableName;
    }
    self.tableView.shouldRecognizeSimultaneouslyWithGestureRecognizer = @(self.shouldRecognizeSimultaneouslyWithGestureRecognizer);
    [self _render];
}
